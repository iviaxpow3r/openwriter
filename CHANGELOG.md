...
### Changed
- **Save-time matcher now broadcasts ID rewrites to all connected clients (architectural fix).** The matcher is the authority on block identity; whenever it reassigns an ID at save time, the server now emits a new `id-rewrites` WebSocket message carrying the `{oldId, newId}[]` mapping. Browser clients walk their in-memory TipTap doc and call `setNodeMarkup` for each match in a single atomic ProseMirror transaction, so the browser converges to the server's authoritative ID assignment after every save. This closes the v0.14.0 bug class at the protocol layer: even when the matcher legitimately reassigns IDs (transient editor-minted IDs at slots with previousNodes orphans, or doc-update reconciliation from a browser that shipped stale state), the browser is told and updates. The matcher guard above narrows how often the broadcast fires; the broadcast itself ensures convergence whenever it does. New listener API: `onIdRewrites(listener)` in `server/state.ts`, new browser export `applyIdRewritesToEditor(editor, rewrites)` in `src/decorations/bridge.ts`.
