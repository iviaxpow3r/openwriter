...
export async function startHttpServer(options: { port?: number; noOpen?: boolean; plugins?: string[] } = {}): Promise<void> {
  ...
  app.get('/api/status', (_req, res) => {
    // Native bundles use this marker to distinguish their current service
    // from an older process left behind by a prior app launch on the same
    // port. It is generated by the launcher from the bundled server asset;
    // it is never a user identifier or writing-space value.
    res.json({
      ...getStatus(),
      ...(process.env.OPENWRITER_SERVICE_BUILD_ID ? { serviceBuildId: process.env.OPENWRITER_SERVICE_BUILD_ID } : {}),
    });
  });
